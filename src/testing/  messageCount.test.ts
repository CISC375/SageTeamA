...
describe('handleFAQResponse', () => {
	...
	it('should process if channel is not in disabled list', async () => {
		mockFindOne
			.mockResolvedValueOnce({ disabledAutoResponseChannels: ['channel2'] }) // Different channel
			.mockResolvedValueOnce(null); // No cooldown

		mockFind.mockReturnValueOnce({
			toArray: () => Promise.resolve([]) // No matching FAQs
		});

		await handleFAQResponse(mockMessage, now);

		// Since we're providing no matching FAQs, we won't get a reply
		// but we can verify the channel check was passed by seeing if the second findOne was called
	...
	it('should handle case where disabledAutoResponseChannels is undefined', async () => {
		mockFindOne
			.mockResolvedValueOnce({}) // No disabledAutoResponseChannels property
			.mockResolvedValueOnce(null); // no cooldown
		...
		// Since there's no disabledAutoResponseChannels, it should proceed to check cooldown
		expect(mockFindOne).toHaveBeenCalledTimes(2);
	});
});
